...
3. **Isolate writes.** For write runs, ensure a clean rollback point: commit/stash first, or use a fresh worktree (`git worktree`, `claude -w`, `grok -w`, or a Codex sandbox). Never launch a write run onto a dirty tree you can't cleanly separate.
