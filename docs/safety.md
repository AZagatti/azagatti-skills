...
1. **Least privilege by default.** Start read-only. Grant writes/command-execution only for the specific task, with the narrowest flag that works:
   ...
   - Antigravity: `--mode plan`/default → a scoped `permissions.allow` rule in `settings.json`. On 1.1.10, `--mode accept-edits` still soft-denies headless writes; `--dangerously-skip-permissions` only when explicitly asked.
