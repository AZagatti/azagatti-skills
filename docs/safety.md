...
1. **Least privilege by default.** Start read-only. Grant writes/command-execution only for the specific task, with the narrowest flag that works:
   ...
   - Claude: configured baseline → `--permission-mode dontAsk --tools "Read,Grep,Glob"` for a locked-down built-in read surface (inherited MCP/hooks/policy still apply), or `acceptEdits` for authorized edits plus common in-scope filesystem commands; `bypassPermissions` only when asked.
   - Grok: `--permission-mode dontAsk` plus an explicit read-only `--tools`/`--allow` surface → expand with scoped rules before considering `--always-approve`/`bypassPermissions`; inherited MCP/hooks/policy still apply.
   - Antigravity: `--mode plan`/default → a temporary exact absolute-target `permissions.allow` rule in `~/.gemini/antigravity-cli/settings.json`. This user-level rule is global and persistent, so preserve prior settings and remove it after the run. On 1.1.10, `--mode accept-edits` still soft-denies headless writes; `--dangerously-skip-permissions` only when explicitly asked.
