...
| Prompt delivery | arg or `-` stdin | arg or stdin | **prompt is `-p`'s value** (ordering gotcha) or `--prompt-file` | prompt is `-p`'s value (aliases `--print` / `--prompt`) |
| Workspace | `-C <dir>` | launch **cwd** (+ `--add-dir`) | `--cwd <dir>` | **none → `--add-dir` required** (else isolated scratch) |
| Default write policy | read-only/configured | configured baseline; prompt-class tools deny unless pre-authorized | trust/settings/rule-dependent; approval prompts deny headlessly | denied in headless, including `--mode accept-edits` on 1.1.10 |
...
| Web search | via separately authorized tools/network | permission/config dependent | permission/config dependent | not documented |
